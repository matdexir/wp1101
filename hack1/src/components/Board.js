...
const Board = ({ boardSize, mineNum, backToHome }) => {
  ...
  const freshBoard = () => {
    ...
    const ret = createBoard(boardSize, mineNum);
		setBoard(board => [...board, ret.board]);
		console.log(typeof(ret.board))
		console.log(board);
		setMineLocations([...ret.mineLocations]);
  };
  ...
  const createRow = () => {
    ...
    for (let i = 0; i < boardSize; i++) {
      ...
      rows.push(
        <div id={row_id} style={{ display: "flex" }}>
        </div>
      );
    }
    return rows;
